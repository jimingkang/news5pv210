...
/*  获得当前特定摄像头参数，但是参数是不正确的，需要后面修改，然后重新设置
...
static int myuvc_get_streaming_params(struct myuvc_streaming_control *ctrl)
{
	...
	/* UVC 版本 */
	size = uvc_version >= 0x0110 ? 34 : 26; //具体操作仿照原生代码  34 26为分配的字节数
	data = kmalloc(size, GFP_KERNEL); //分配缓存空间
	if (data == NULL)
		return -ENOMEM;
   
   /* 发起USB传输  全部参考UVC框架 */
   /*
   usb 传输端点 管道  GET_CUR：0x81 对比源码猜测是获取当前信息，GET_CUR & 0x80 为真,
   选择 usb_rcvctrlpipe(myuvc_udev, 0)， 属于 rcv 类型 ，即接受类型。 0 代表端点0，
   我们的这些参数都是从端点0获得的。
	*/
	...
	/*
	GET_CUR:获取当前信息，
	VS_PROBE_CONTROL ：枚举参数
	myuvc_streaming_intf ： usb 摄像头有两个Interface : VC  VS。这些参数是在VS中的 所以选择VS
	*/
	//发起一个USB传输  获取 GET_CUR 当前所使用的参数，存储在data里面。
	...
	//将存储在 data 中的当前参数信息 存储到 myuvc_streaming_control的各个对应项中。
...
static int myuvc_try_streaming_params(struct myuvc_streaming_control *ctrl)
{
	...
	memset(ctrl, 0, sizeof *ctrl);//清零
    
	/*可以查找 uvc 规范*/
	
	/*
	ctrl->bmHint = 1; ==> dwFrameInterval 表示此处是协商过程，表示我们设置一些参数发给摄像头，摄像头测试这些参数能不能用，如果能用,我们将
	这些参数用不同的命令发给摄像头。接受新参数，并且工作于新参数之下
	
	ctrl->bFormatIndex = 1; 目前只支持一种格式 选择1
	ctrl->bFrameIndex  = frame_idx + 1; 使用那一种分辨率，我们的驱动分辨率固定是 {352, 288} 所以为 frame_idx + 1;
	ctrl->dwFrameInterval = 333333; 最小帧间隔，每一帧之间的时间间隔 单位是100ns  ，lsusb 为 333333 ，那么一秒： 1000000000ns/（333333*100ns）=30,即
		一秒 30帧
	*/
	ctrl->bmHint = 1;	
	ctrl->bFormatIndex = 1;
...
//发出usb传输方式 和 myuvc_get_streaming_params()大致一致，只是传输表示从  GET_CUR 变成 SET_CUR，
    ...
    *(__le16 *)&data[16] = cpu_to_le16(ctrl->wDelay);
	...
	//发起一个USB传输，用于测试 设置 参数，所以选择为 SET_CUR，但是并不是真的设置参数，只是测试，所以选择 VS_PROBE_CONTROL(枚举)，而并非(VS_COMMIT_CONTROL)提交。
    //真正设置参数的时候 一定要改成 VS_COMMIT_CONTROL
...
static int myuvc_set_streaming_params(struct myuvc_streaming_control *ctrl)
{
    ...
    *(__le16 *)&data[16] = cpu_to_le16(ctrl->wDelay);
	...
	//发起一个usb传输，VS_COMMIT_CONTROL类型，提交参数
...
static int myuvc_vidioc_streamon(struct file *file, void *priv, enum v4l2_buf_type i)
{
     ...
     * 	1.1 根据结构体 uvc_streaming_control 设置数据包: 可以手工设置,也可以读出后再修改
     * 	1.2 调用 usb_control_msg 发出数据包
     */

	 /* a b c 步骤逻辑
	 a 步骤 测试参数，代码中只是测试设置了某几个参数，并没有将全部的参数都测试一遍，发给USB设备之后，USB设备发现可以接收，他就会补齐其他的参数
	 b 步骤 所以 就可以 用 myuvc_get_streaming_params()获取补齐后的usb摄像头参数
	 c 步骤 最后调用 myuvc_set_streaming_params() 设置参数
	 */
    ...
    /* b. 取出参数  获得当前特定摄像头参数，但是参数是不正确的，需要后面修改，然后重新设置*/
...
/*
 每个 usb interface里面有多个设置(setting)，那么我们使用哪一个设置呢？根据端点能传输的最大带宽容量来选择 setting
*/
    /* d. 设置 VideoStreaming Interface所使用的 setting
     * 	d.1 从 myuvc_params 确定带宽
     * 	d.2 根据 setting 的 endpoint端点 能传输的 wMaxPacketSize最大包 找到能满足该带宽的 setting   
     */
    /* 手工确定:
     * bandwidth = myuvc_params.dwMaxPayloadTransferSize = 1024
     * 观察lsusb -v -d 0x1e4e:的结果: 能够满足 dwMaxPayloadTransferSize的是：
     *                wMaxPacketSize     0x0400  1x 1024 bytes
     * 				  bAlternateSetting       8
	   所以 自定义 myuvc_streaming_bAlternateSetting = 8，即选择第八个setting
     */
