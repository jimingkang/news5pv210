...
static struct snd_soc_dai_ops s3c64xx_i2s_dai_ops0;
static struct snd_soc_dai_ops s3c64xx_i2s_dai_ops1;
...
bool audio_clk_gated0;
bool audio_clk_gated1;

/* For I2S Clock/Power Gating */
static int tx_clk_enabled0 ;
static int rx_clk_enabled0 ;
static int reg_saved_ok0 ;
static int tx_clk_enabled1 ;
static int rx_clk_enabled1 ;
static int reg_saved_ok1 ;
...
void s5p_i2s_set_clk_enabled0(struct snd_soc_dai *dai, bool state)
{
...
void s5p_i2s_set_clk_enabled1(struct snd_soc_dai *dai, bool state)
{
	struct s3c_i2sv2_info *i2s = to_info(dai);
...
static int s5p_i2s_wr_startup0(struct snd_pcm_substream *substream,
		struct snd_soc_dai *dai)
{
	struct s3c_i2sv2_info *i2s = to_info(dai);
	u32 iiscon, iisfic;

	if (!tx_clk_enabled0 && !rx_clk_enabled0) {
		s5p_i2s_set_clk_enabled0(dai, 1);
		if (reg_saved_ok0 == true) {
...
static int s5p_i2s_wr_startup1(struct snd_pcm_substream *substream,
		struct snd_soc_dai *dai)
{
	struct s3c_i2sv2_info *i2s = to_info(dai);
	u32 iiscon, iisfic;

	if (!tx_clk_enabled1 && !rx_clk_enabled1) {
		s5p_i2s_set_clk_enabled1(dai, 1);
		if (reg_saved_ok1 == true) {
			/* Is this dai for I2Sv5? (I2S0) */
			if (dai->id == 0) {
				writel(i2s->suspend_audss_clksrc,
						S5P_CLKSRC_AUDSS);
...
static void s5p_i2s_wr_shutdown0(struct snd_pcm_substream *substream,
		struct snd_soc_dai *dai)
{
	struct s3c_i2sv2_info *i2s = to_info(dai);
	...
	if (!tx_clk_enabled0 && !rx_clk_enabled0) {
		...
		reg_saved_ok0 = true;
		s5p_i2s_set_clk_enabled0(dai, 0);
...
static void s5p_i2s_wr_shutdown1(struct snd_pcm_substream *substream,
		struct snd_soc_dai *dai)
{
	struct s3c_i2sv2_info *i2s = to_info(dai);
	...
	if (!tx_clk_enabled1 && !rx_clk_enabled1) {
		...
		reg_saved_ok1 = true;
		s5p_i2s_set_clk_enabled1(dai, 0);
		pr_debug("I2S Audio Clock disabled and Registers stored...\n");
		pr_debug("Inside %s CLkGATE_IP3=0x%x..\n",
				__func__ , __raw_readl(S5P_CLKGATE_IP3));
	}
...
static void s3c64xx_iis_dai_init(struct snd_soc_dai *dai)
{
	if(dai->id == 0)
	{
		printk("s3c64xx_iis_dai_init:s3c64xx-i2s0 \n");
		dai->name = "s3c64xx-i2s0";
		...
		dai->ops = &s3c64xx_i2s_dai_ops0;
	}
	else if(dai->id == 1)
	{
		printk("s3c64xx_iis_dai_init:s3c64xx-i2s1 \n");
		dai->name = "s3c64xx-i2s1";
		dai->symmetric_rates = 0;
		dai->playback.channels_min = 1;	/* I2S dai : match Playback channels_min value with wm8580_dai(@wm8580.c) */
		dai->playback.channels_max = 2;
		dai->playback.rates = S3C64XX_I2S_RATES;
		...
		dai->capture.rates = S3C64XX_I2S_RATES;
		dai->capture.formats = S3C64XX_I2S_FMTS;
		dai->ops = &s3c64xx_i2s_dai_ops1;
	}
}

/* suspend/resume are not necessary due to Clock/Pwer gating scheme... */
#ifdef CONFIG_PM
static int s5p_i2s_suspend0(struct snd_soc_dai *dai)
{
	struct s3c_i2sv2_info *i2s = to_info(dai);
...
static int s5p_i2s_resume0(struct snd_soc_dai *dai)
{
	struct s3c_i2sv2_info *i2s = to_info(dai);
...
static int s5p_i2s_suspend1(struct snd_soc_dai *dai)
{
	struct s3c_i2sv2_info *i2s = to_info(dai);
...
static int s5p_i2s_resume1(struct snd_soc_dai *dai)
{
	struct s3c_i2sv2_info *i2s = to_info(dai);
...
#else
#define s5p_i2s_suspend0 NULL
#define s5p_i2s_resume0  NULL
#define s5p_i2s_suspend1 NULL
#define s5p_i2s_resume1  NULL
#endif	/* CONFIG_PM */
...
int s5p_i2sv5_register_dai(struct snd_soc_dai *dai)
{
	...
	if(dai->id == 0)
	{
		ops->startup   = s5p_i2s_wr_startup0;	
		ops->shutdown = s5p_i2s_wr_shutdown0;
		dai->suspend = s5p_i2s_suspend0;
		dai->resume = s5p_i2s_resume0;
	}
	else if(dai->id == 1)
	{
		ops->startup   = s5p_i2s_wr_startup1;	
		ops->shutdown = s5p_i2s_wr_shutdown1;
		dai->suspend = s5p_i2s_suspend1;
		dai->resume = s5p_i2s_resume1;
	}
...
static __devinit int s3c64xx_iis_dev_probe(struct platform_device *pdev)
{
	...
	writel(1<<31, i2s->regs + S3C2412_IISCON);
#endif
	...
	if (pdev->id == 0)
	{
		printk("s3c64xx_iis_dev_probe:id0 !!!\n");
