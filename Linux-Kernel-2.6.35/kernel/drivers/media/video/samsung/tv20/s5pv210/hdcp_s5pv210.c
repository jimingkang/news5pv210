...
#define HDCPPRINTK(fmt, args...) \
	pr_debug(KERN_INFO "\t\t[HDCP] %s: " fmt, __func__ , ## args)
#else
#define HDCPPRINTK(fmt, args...)
#endif
...
#define AUTHPRINTK(fmt, args...) \
	pr_debug("\t\t\t[AUTHKEY] %s: " fmt, __func__ , ## args)
#else
#define AUTHPRINTK(fmt, args...)
#endif
...
static bool compare_r_val(void)
{
	...
	for (i = 0; i < R_VAL_RETRY_CNT; i++) {
		...
		AUTHPRINTK("\t\t\t Rx(ddc) ->");
		AUTHPRINTK("rj[0]: 0x%02x, rj[1]: 0x%02x\n",
			rj[0], rj[1]);
		AUTHPRINTK("\t\t\t Tx(register) ->");
		AUTHPRINTK("ri[0]: 0x%02x, ri[1]: 0x%02x\n",
			ri[0], ri[1]);
#endif
		/* Compare R value */
...
static int hdcp_loadkey(void)
{
	...
	if (time_out <= 0) {
		if (readb(g_hdmi_base + S5P_EFUSE_STATUS) & EFUSE_ECC_FAIL) {
			...
			/*
			pr_err("%s::readb S5P_EFUSE_STATUS for EFUSE_ECC_DONE fail\n",
				__func__);
			*/
		}
		return -EINVAL;
