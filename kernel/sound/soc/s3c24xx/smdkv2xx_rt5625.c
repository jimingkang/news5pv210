...
/* yman.seo XCLK_OUT control removed.
   XCLK_OUT drives wm8580's XTI, which should be a stable 12MHz or 24MHz clock */
#if 0
	...
	struct snd_pcm_hw_params *params)
{
	...
	printk("Entered smdkc100_hw_params 135\n");
...
// fs==8K for recording, fs==48k for playback, need to change
#if 1
	printk("set up rclk:512*%d substream:%d\n",rclk/rfs,substream->stream);
#else
	if(rt5625_record_status==RT5625_CAPTURE){
		rclk=512*8000;
		printk("set up rt5625 for recording\n");
	}
	else{
		rclk=512*48000;
		printk("set up rt5625 for playing\n");
	}
#endif		
	...
	ret = snd_soc_dai_set_sysclk(codec_dai, RT5625_MCLK,
					epll_out_rate/psr, SND_SOC_CLOCK_OUT);
...
#if 1
	ret = snd_soc_dai_set_fmt(codec_dai, (SND_SOC_DAIFMT_CBM_CFM |SND_SOC_DAIFMT_I2S |SND_SOC_DAIFMT_NB_NF));
	if (ret < 0)
		return ret;
...
/* Module information */
MODULE_DESCRIPTION("ALSA SoC SMDKC100 RT5625");
MODULE_LICENSE("GPL");
