		...
		struct snd_pcm_hw_params *params)
{
	...
	pr_debug("%s:card->id=%s,number=%d,driver=%s\n", __func__,*substream->pcm->card->id,substream->pcm->card->number,*substream->pcm->card->driver);
...
//#ifdef CONFIG_S5P_INTERNAL_DMA
//if(substream->pcm->card->number == 1)
//{
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK)
	{
		if(substream->pcm->card->number == 0)
		...
		struct snd_soc_dai *dai, struct snd_pcm *pcm)
{
	...
	pr_debug("%s:id=%d,name=%s,card->id=%s,number=%d,driver=%s\n", __func__,dai->id,dai->name,*card->id,card->number,*card->driver);
