...
#ifdef CONFIG_OLD_DMA_PL330	/* DMA auto-reload Micro Code */
#define PLAYBACK_INFINITYLOOP
#define CAPTURE_INFINITYLOOP
#endif
...
static void s3c_dma_enqueue(struct snd_pcm_substream *substream)
{
	...
	unsigned int limit;
	int ret;
	int use_dma_infinityloop = 0;

	pr_debug("Entered %s\n", __func__);

#ifdef PLAYBACK_INFINITYLOOP
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK)
		use_dma_infinityloop = 1;
#endif
#ifdef CAPTURE_INFINITYLOOP
	if (substream->stream == SNDRV_PCM_STREAM_CAPTURE)
		use_dma_infinityloop = 1;
#endif

	if (use_dma_infinityloop) {
		/* Force to use 4periods DMA auto-reload
		   dma_enqueue should be called only one time for full length. */
		len = substream->runtime->dma_bytes;
		limit = 1;
	} else {
		len = prtd->dma_period;
...
#if !defined(PLAYBACK_INFINITYLOOP) && !defined(CAPTURE_INFINITYLOOP)
...
static void s3c_dma_free_dma_buffers(struct snd_pcm *pcm)
{
	...
	pr_debug("%s:card->id=%s,number=%d,driver=%s\n", __func__,*pcm->card->id,pcm->card->number,*pcm->card->driver);

//#ifdef CONFIG_S5P_INTERNAL_DMA
if(pcm->card->number == 1)
	stream = 0;
else if(pcm->card->number == 0)
	stream = 1;
	//for (stream = 1; stream < 2; stream++) {
//#else
	for (stream; stream < 2; stream++) {
//#endif
